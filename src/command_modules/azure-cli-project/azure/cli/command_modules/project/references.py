...
def reference_exists(service_name, reference_name, namespace_name):
    """
    Checks if secret for provided service
    and reference exists or not.
    """
    secret_name = get_secret_name(service_name, reference_name)
    command = "kubectl get secret {} -n {}".format(secret_name, namespace_name)
...
def add_reference(
    service_name, target_group, target_name,
    reference_name, env_variables, namespace_name,
    reference_type):
    """
    ...
    if reference_type == 'custom' and not target_group and not target_name:
        ...
        created_variables = _create_custom_reference(
            service_name, reference_name, dict_vars, namespace_name)
    else:
        instance, client = get_reference_type(target_group, target_name, reference_type)
        ...
        if instance_type == 'Microsoft.DocumentDB/databaseAccounts':
            ...
            created_variables = create_connection_string_reference(
                service_name, reference_name, connection_string, namespace_name)
            ...
            created_variables = create_username_password_reference(
                service_name, reference_name, sql_admin_login,
                sql_admin_password, fqdn, namespace_name)
...
def _create_custom_reference(
    service_name, reference_name, key_value_pairs, namespace_name):
    """
    ...
    command = 'kubectl create secret generic {} {} -n {}'.format(
        secret_name, ' '.join(literals), namespace_name)
...
def create_connection_string_reference(
    service_name, reference_name, connection_string, namespace_name):
    """
    ...
    _create_custom_reference(service_name, reference_name, pairs, namespace_name)

    return [environment_variable_name]


def create_username_password_reference(
    service_name, reference_name, admin_login,
    admin_password, fqdn, namespace_name):
    """
    Creates a secret on Kubernetes that stores
    ...
    environment_variables = _create_custom_reference(
        service_name, reference_name, pairs, namespace_name)
...
def get_reference_type(resource_group, resource_name, reference_type):
    """
    Gets the reference type from the provided
    resource group and name
    """
    instance = None
    if reference_type == 'mongo':
        try:
        ...
        try:
            ...
            pass
    elif reference_type == 'servicebus':
        try:
            ...
            return NamespacesOperations.type, namespaces
        except Exception as exc:
            raise CLIError(exc)
